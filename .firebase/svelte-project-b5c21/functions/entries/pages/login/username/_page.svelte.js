...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(AuthCheck, "AuthCheck").$$render($$result, {}, {}, {
    default: () => {
      return `${$userData?.username ? `<p class="text-lg">Your username is <span class="font-bold text-green-400">@${escape($userData.username)}</span></p> <p class="text-sm" data-svelte-h="svelte-9l80or">(Usernames cannot be changed)</p> <a class="btn btn-primary mt-3" href="/login/photo" data-svelte-h="svelte-1ynqfz">Upload Profile Image</a>` : `<form class="w-2/5"><input type="text" placeholder="Username" class="${[
        "input w-full",
        (!isValid && isTouched ? "input-error" : "") + " " + (isTaken ? "input-warning" : "") + " " + (isValid && isAvailable && !loading ? "input-success" : "")
      ].join(" ").trim()}"${add_attribute("value", username, 0)}> <div class="my-4 min-h-16 px-8 w-full">${``} ${!isValid && isTouched ? `<p class="text-error text-sm" data-svelte-h="svelte-15rk0xx">must be 3-16 characters long, alphanumeric only</p>` : ``} ${isValid && !isAvailable && !loading ? `<p class="text-warning text-sm">@${escape(username)} is not available</p>` : ``} ${isValid && isAvailable && !loading ? `<button class="btn btn-success">Confirm username @${escape(username)}</button>` : ``}</div></form>`}`;
    }
  })}`;
});
export {
